...
* **Tier 2 — small experts.** creatures (folding their powers + intents into one expert), relics
  (set-membership head), potions (per-slot categorical — potions can duplicate), orbs.
...
# ==================================================================================================
# Relic expert (set-membership head — duplicate-free by construction).
# ==================================================================================================
...
class RelicExpert(nn.Module):
    ...
    def __init__(self, name: str, latent_width: int, d_model: int,
        ...
        self.d_model = d_model
        self.n_mem = n_mem
        self.embedder = _TypeEmbedder(self.tspec, d_model, cat_dim, static_tables)
        self.cls = nn.Parameter(torch.randn(d_model) * 0.02)   # always-valid anchor (see SetExpert.cls)
        enc_layer = nn.TransformerEncoderLayer(d_model, n_heads, dim_feedforward=d_model * ff_mult,
                                               batch_first=True, norm_first=True, activation="gelu")
        self.enc_trunk = nn.TransformerEncoder(enc_layer, enc_layers)
        self.latents = nn.Parameter(torch.randn(pool_latents, d_model) * 0.02)
    ...
    def encode(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        device = batch["relic_idx"].device
        emb = torch.cat([self.cls.view(1, 1, -1).expand(B, 1, -1),
                         self.embedder(batch["relic_idx"], None, None)], dim=1)
        key_pad = torch.cat([torch.zeros(B, 1, dtype=torch.bool, device=device),
                             ~batch["relic_mask"]], dim=1)
        toks = self.enc_trunk(emb, src_key_padding_mask=key_pad)
        B = toks.shape[0]
        lat = self.latents.unsqueeze(0).expand(B, -1, -1)
        for layer in self.pool:
            lat = layer(lat, toks, key_pad)
