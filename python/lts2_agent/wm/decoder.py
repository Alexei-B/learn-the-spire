...
class _TypeHeads(nn.Module):
    ...
    Relics (v5) decode through the shared per-slot categorical path like every other set type — their
    `slot` categorical carries acquisition order (positional), so the old duplicate-free multi-hot set
    head is gone."""
